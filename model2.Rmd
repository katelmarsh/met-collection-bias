...
culture <- head(sort(table(df$culture), decreasing = TRUE), 20) # the culture section could clearly use some cleaning! japan and japanese haha 
...
ggplot(c_df, aes(x = Freq, y = Var1)) + 
  geom_col() + theme_light() + ggtitle(label = "Art Culture, Top 20") 
...
fit <- rpart(is.highlightTrue~., data = train_dat, 
             method = 'class', 
             control = rpart.control(cp = .01))

summary(fit)

train_dat$pred <- predict(fit, train_dat, type = "class")
(accuracy <- mean(train_dat$pred == train_dat$is.highlightTrue))

test_dat$pred <- predict(fit, test_dat, type = "class")
(accuracy <- mean(test_dat$pred == test_dat$is.highlightTrue))
